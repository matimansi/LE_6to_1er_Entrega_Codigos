...
void conversor (void)
{
    ...
    analisis.num = numero;
    printf("\n%f = 0x", numero);
    for (i = 3; i >= 0; i--) printf("%.2X", analisis.byte[i]);
    printf(" = 0b");
    for (i = 3; i >= 0; i--) for (j = 7; j >= 0; j--) printf("%d", ((analisis.byte[i] >> j) & 1));
    printf("\n\n--- Partes del IEEE 754 ---\n\n");
    printf("Signo = 0b%d", ((analisis.byte[3] >> 7) & 1));
    printf("\nExponente = 0b");
    for (i = 6; i >= 0; i--) printf("%d", ((analisis.byte[3] >> i) & 1));
    printf("%d", ((analisis.byte[2] >> 7) & 1));
    printf("\nMantisa = 0b");
    for (i = 6; i >= 0; i--) printf("%d", ((analisis.byte[2] >> i) & 1));
    for (i = 7; i >= 0; i--) printf("%d", ((analisis.byte[1] >> i) & 1));
    for (i = 7; i >= 0; i--) printf("%d", ((analisis.byte[0] >> i) & 1));
}
...
void imprimir (void)
{
    int i;
    printf("= 0x");
    for (int i = 3; i >= 0; i--) printf("%.2X", analisis.byte[i]);
    printf(" = 0b");
    for (int i = 3; i >= 0; i--) for (int j = 7; j >= 0; j--) printf("%d", ((analisis.byte[i] >> j) & 1));
    printf("\n\n--- Partes del IEEE 754 ---\n\n");
    printf("Signo = 0b%d", ((analisis.byte[3] >> 7) & 1));
    printf("\nExponente = 0b");
    for (i = 6; i >= 0; i--) printf("%d", ((analisis.byte[3] >> i) & 1));
